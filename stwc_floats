...
__global__ void single_thread(float *sh_sum_dev, float *weight_dev, float *values_dev, float W, float *str_num_dev, float num_of_blocks,int rep)
{
  float th_w_sum = 0;
  float th_v_sum = 0;
...
int main(){
  ...
  cudaMalloc ((void **) &weight_dev, arraySize * sizeof (float));
  cudaMalloc ((void **) &values_dev, arraySize * sizeof (float));
  cudaMalloc ((void **) &str_num_dev, num_of_blocks * sizeof (float));

  cudaMemcpy (weight_dev, weight, arraySize * sizeof (float),
	      cudaMemcpyHostToDevice);
  cudaMemcpy (values_dev, values, arraySize * sizeof (float),
	      cudaMemcpyHostToDevice);
