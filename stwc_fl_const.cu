...
__global__ void single_thread(float *sh_sum_dev,float W, float *str_num_dev, float num_of_blocks,int rep)
{
  ...
  //__shared__ float sh_w_d[arraySize];
  //__shared__ float sh_v_d[arraySize];
...
if(threadIdx.x<arraySize){
	sh_w_d[threadIdx.x] = coefs[threadIdx.x];
	sh_v_d[threadIdx.x] = coefs[threadIdx.x+arraySize];
}
...
int main(){
  ...
  float strSize_b = pow (2, arraySize);
  float num_of_blocks = strSize_b / threads_per_block;
  float *Sum = new float[1];	// = { 0 };
  float *sh_sum_dev;
  float *weight_dev;
  float weight[30] ={ 5, 10, 17, 19, 20, 23, 26, 30, 32, 38, 40, 44, 47, 50, 55, 56, 56, 60, 62, 66, 70, 75, 77, 80, 81, 90,93,96,101,107 };
  float values[30] ={ 10, 13, 16, 22, 30, 25, 55, 90, 110, 115, 130, 120, 150, 170, 194, 199, 194, 199, 217, 230, 248, 250, 264, 271, 279, 286,293,299,305,313 };
  ...
  cout<<"red param "<<(num_of_blocks-1) / threads_per_block<<"  ,  "<<strSize_b/num_of_blocks<<"\n";
  ...
  //cudaMalloc ((void **) &weight_dev, 2*arraySize * sizeof (float));
  //cudaMalloc ((void **) &values_dev, arraySize * sizeof (float));

  cudaMalloc ((void **) &str_num_dev, num_of_blocks * sizeof (float));

  cudaMemcpyToSymbol (coefs, dev_coefs, 2*arraySize * sizeof (float));
  //cudaMemcpy (values_dev, values, arraySize * sizeof (float), cudaMemcpyHostToDevice);


        for(int i = 0;i<N_of_rep;i++){
  single_thread <<< num_of_blocks/N_of_rep, threads_per_block >>> (sh_sum_dev, W, str_num_dev, num_of_blocks,i);
             }
//cout all the parameters


  reduction_max <<<(num_of_blocks-1) / threads_per_block, strSize_b/num_of_blocks>>> (sh_sum_dev, str_num_dev);
  reduction_max <<<1,(num_of_blocks-1) / threads_per_block>>> (sh_sum_dev, str_num_dev);
